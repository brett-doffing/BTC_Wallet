...
public struct BTCTransaction {
    ...
    */
    func createTX(
        ...
        rawTX += numAddresses
        appendAmountAndOutputScript(&rawTX, satoshis, receivingAddresses, opReturnData)
    ...
    private func appendAmountAndOutputScript(
        _ rawTX: inout Data,
        _ satoshis: [UInt64],
        _ receivingAddresses: [String],
        _ opReturnData: Data? = nil
    ) {
        var counter = 0
        while counter < receivingAddresses.count {
            if receivingAddresses[counter] == "" {
                guard let opReturnData = opReturnData else { return }
                var satoshis = satoshis[counter].littleEndian
                rawTX += Data(bytes: &satoshis, count: MemoryLayout<UInt64>.size)
                rawTX += UInt8(0x53) // next bytes length?
                rawTX += OP_RETURN
                rawTX += OP_PUSHDATA1
                rawTX += UInt8(0x50) // next bytes length?
                rawTX += opReturnData
            } else {
                var satoshis = satoshis[counter].littleEndian
                rawTX += Data(bytes: &satoshis, count: MemoryLayout<UInt64>.size)
