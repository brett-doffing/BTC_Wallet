...
class BTCCurveTests: XCTestCase {
    func test_BTCCurve_publicKey_compressed() {
        for i in 0..<prvkeys.count {
            ...
            XCTAssertEqual(publicKey.hexDescription(), pubkeys[i])
        }
    }
    ...
    func test_BTCCurve_publicKey_uncompressed() {
        for i in 0..<prvkeys.count {
            ...
            XCTAssertEqual(publicKey.hexDescription(), uncompressedPubkeys[i])
        }
    }
    ...
    func test_BTCCurve_signature() {
        ...
        XCTAssertEqual(signature.r.hexDescription(), r)
        XCTAssertEqual(signature.s.hexDescription(), s)
    }
